...
export default function DoctorForm({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: `Failed to ${isNew ? "add" : "update"} doctor. Please try again.`,
        variant: "destructive",
      })
    } finally {
      setIsSaving(false)
      ...
      <main className="flex-1 p-6 bg-gray-50">
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center mb-6">
            ...
            <h1 className="text-3xl font-bold">{isNew ? "Add Doctor" : "Edit Doctor"}</h1>
          </div>

          <Card>
            <CardHeader>
              <CardTitle>{isNew ? "New Doctor" : "Edit Doctor"}</CardTitle>
              <CardDescription>
                {isNew ? "Add a new doctor to your medical practice." : "Update the doctor's information."}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-end gap-4">
                  ...
                  <Button type="submit" disabled={isSaving}>
                    {isSaving ? "Saving..." : isNew ? "Add Doctor" : "Update Doctor"}
                  </Button>
                </div>
              </form>
            </CardContent>
